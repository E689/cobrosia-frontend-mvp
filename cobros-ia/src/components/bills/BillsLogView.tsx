...
export default function BillsLogView({ billId, expanded, setExpanded }) {
    // Vars
    ...
    const loadLog = useCallback(() => {
        try {
            ...
            toast.warn("Parece que algo salio mal...")
        }
    }, [billId])
    ...
    useEffect(() => {
        if (expanded && billId) loadLog
    }, [expanded, billId, loadLog])
                                ...
                                if (entry.user === "GPT") {
                                    return (
                                        <div className="flex flex-row" key={entry._id}>
                                            ...
                                            <div className="flex max-w-[85%] h-full border border-brand-color rounded-md p-1">
                                    ...
                                    return (
                                        <div className="flex flex-row-reverse" key={entry._id}>
                                            ...
                                            <div className="flex max-w-[85%] h-full border border-classy-blue rounded-md p-1">
                                                <span className="ml-auto pr-1 text-sm text-wrap">{entry.msg}</span>
                                            </div>
                                        </div>
                                    )
                                }
